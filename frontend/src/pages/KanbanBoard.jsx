...
export default function KanbanBoard() {
  ...
  const fetchKanbanData = async () => {
    try {
      ...
      // If data is an array, group by stage
      if (Array.isArray(data)) {
        data.forEach(request => {
          const stage = request.stage || 'New';
          if (kanbanStructure[stage]) {
            kanbanStructure[stage].push(request);
          }
        });
        setKanbanData(kanbanStructure);
      } else if (data && typeof data === 'object') {
        // If data is already grouped
        setKanbanData({ ...kanbanStructure, ...data });
      } else {
        setKanbanData(kanbanStructure);
      }
    } catch (error) {
  ...
  const handleDragEnd = async (result) => {
    ...
    const sourceItems = Array.from(newKanbanData[source.droppableId] || []);
    const destItems = source.droppableId === destination.droppableId 
      ? sourceItems 
      : Array.from(newKanbanData[destination.droppableId] || []);
    
    const [movedItem] = sourceItems.splice(source.index, 1);
    destItems.splice(destination.index, 0, { ...movedItem, stage: destination.droppableId });
    
    newKanbanData[source.droppableId] = sourceItems;
    newKanbanData[destination.droppableId] = destItems;
    
    setKanbanData(newKanbanData);

    try {
      console.log('Updating request stage:', draggableId, 'to', destination.droppableId);
      const response = await maintenanceRequestAPI.updateRequestStage(draggableId, {
      ...
      toast.error(`Failed to update request stage: ${error.response?.data?.message || error.message}`);
      ...
      <Draggable draggableId={request._id} index={index}>
        {(provided, snapshot) => (
          <div
            ...
            className={`p-5 mb-4 bg-white rounded-2xl border-2 cursor-pointer transition-all duration-200 ${
              stageColors[request.stage]
            } ${snapshot.isDragging ? 'shadow-2xl scale-105 rotate-2' : 'shadow-lg hover:shadow-xl'} ${
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center space-x-3">
                        <div className={`p-2 rounded-xl ${
                          stage === 'New' ? 'bg-gradient-to-r from-amber-500 to-yellow-500' :
                      ...
                      <span className={`px-3 py-1 text-sm font-bold rounded-full ${
                        stage === 'New' ? 'bg-gradient-to-r from-amber-100 to-yellow-100 text-amber-800' :
                    ...
                    <Droppable droppableId={stage}>
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={`min-h-[300px] rounded-2xl p-3 transition-all duration-200 ${
