...
export default function CalendarView() {
  ...
  const fetchCalendarEvents = useCallback(async () => {
    try {
      ...
      // Transform the data to match FullCalendar format
      const transformedEvents = Array.isArray(eventsData) ? eventsData.map(event => ({
        id: event._id || event.id,
        title: event.subject || event.title,
        start: event.scheduledDate || event.start,
        end: event.scheduledDate || event.end,
        extendedProps: {
      ...
      // Only show error if we're not on initial load
      if (!loading) {
  ...
  const upcomingEvents = useMemo(() => {
    ...
    return events
      .filter(event => {
        ...
        return eventDate >= now && event.extendedProps.type === 'Preventive';
      })
  ...
  const overdueTasks = useMemo(() => {
    ...
    return events.filter(event => {
      ...
      return eventDate < now && 
             event.extendedProps.stage === 'New' && 
             event.extendedProps.type === 'Preventive';
    });
  }, [events]);
  ...
  const handleCreateEvent = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Failed to create maintenance event';
        ...
        <div className="bg-white rounded-lg">
          <FullCalendar
            ...
            eventContent={(eventInfo) => {
              return {
                html: `<div class="p-1">
                  <div class="font-semibold text-xs">${eventInfo.event.title}</div>
                  <div class="text-xs opacity-75">${eventInfo.event.extendedProps.equipment?.name || ''}</div>
                </div>`
              };
            }}
          />
